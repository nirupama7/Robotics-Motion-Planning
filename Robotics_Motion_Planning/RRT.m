...
qMilestones = rrtSmoothPath(rob,qMilestones1,sphereCenter,sphereCenter2,...
sphereCenter3,sphereCenter4,sphereCenter5,sphereRadius);

for i=2:size(qMilestones,1)
    T6=rob.fkine(qMilestones(i-1,:));
    a=transl(T6);
    b=a';
    T10=rob.fkine(qMilestones(i,:));
    v=transl(T10);
    w=v';
    line([b(1), w(1)], [b(2), w(2)], [b(3), w(3)], 'Color', 'r','LineWidth', 3);            
            hold on;
end
elapsedTimeRRT=toc
end
